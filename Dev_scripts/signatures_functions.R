...
mix.protocol = function(working.dir,mix_set,spill,alpha=0.5,do.ciber=TRUE,do.known=TRUE,do.mcp=FALSE,rnaseq=TRUE,transpose=FALSE,ct_order=NULL,font.size=7,show.before=TRUE,beta_dist) {
  ...
  #rownamesM2  =intersect(rownames(rxbc),rownames(M2))
  ...
  txt1 = paste(mix_set,mean(diag(rxba)),ddiag,drest,sum(badCor),diff_expected,sum(badCor2),sum(badCor1),sum(badCor1 & badCor2),countNotExpectedCellTypes(M,rownames(dist),families),countNotExpectedCellTypes(M2,rownames(dist),families),sep='\t')

  #file_shuff = paste0(working.dir,'/simulations/mix_',mix_set,'_pvals.txt')
  #if (!file.exists(file_shuff)) {
  #  mix=(read.table(paste0(working.dir,'/simulations/mix_',mix_set,'_expr.txt'),sep="\t",header=TRUE,row.names=1, as.is=TRUE))
  #  res = xCellSignifcance(mix,M2,spill=spill,alpha=alpha,file.name = file_shuff)
  #}
  #pvals=(read.table(file_shuff,sep="\t",header=TRUE,row.names=1, as.is=TRUE))

  pvals = xCellSignifcanceBetaDist(M2,spill$beta_params)

  M3 = M2
  pval.thres = 0.05
  M3[pvals>pval.thres]=0
  rx_pval=cor(t(M3[ct_order,]),t(dist[ct_order,]),method="pearson")
  rx_pval[is.na(rx_pval)] = 0
  pdf(paste0(working.dir,'/simulations/plots/',mix_set,'_spill_pvals.pdf'),width=4,height=4)
  corrplot(as.matrix(rx_pval),tl.cex = font.size/10,tl.col = "black",col=colorRampPalette(rev(colors))(200),order=oclust)
  dev.off()
  A = intersect(rownames(M3),rownames(dist))
  #mixScatters(paste0(working.dir,'/simulations/plots/scatters/',mix_set,'_scatters_spill_pvals.pdf'),M3[A,],dist[A,],norm=FALSE)
  #mixScatters(paste0(working.dir,'/simulations/plots/scatters/spill/',mix_set,'_scatters_spill_pvals.pdf'),M3[A,],dist[A,],norm=FALSE)

  rxba_pval=corrplot(as.matrix(rx_pval[ct_order,ct_order]),tl.cex = font.size/10,tl.col = "black",col=colorRampPalette(rev(colors))(200),order=oclust)

  ddiag = sum(diag(rxba_pval))/sum(diag(rxbc))
  drest = (sum(abs(rxba_pval))-sum(diag(rxba_pval)))/(sum(abs(rxbc))-sum(diag(rxbc)))
  ddist = cor(t(dist))

  badCor = rxba_pval>0.25 | rxbc>0.25
  diag(badCor)<-FALSE
  diff_expectedA = rxba_pval[badCor] - ddist[badCor]

  diff_expected = sum(abs(diff_expectedA))/sum(abs(diff_expectedB))

  badCor1 = rxba_pval>0.25
  diag(badCor1)<-FALSE

  diff_expectedA = rxba_pval[badCor] - ddist[badCor]

  #message(paste(txt1,mix_set,mean(diag(rxba_pval)),ddiag,drest,sum(badCor),diff_expected,sum(badCor2),sum(badCor1),sum(badCor1 & badCor2),countNotExpectedCellTypes(M3,rownames(dist),families),sep='\t'))

  #message(paste(mix_set,mean(diag(rxba)),mean(diag(rxba_pval)),(sum(abs(rxba))-sum(diag(rxba)))/(ncol(rxba)*(ncol(rxba)-1)),(sum(abs(rxba_pval))-sum(diag(rxba_pval)))/(ncol(rxba_pval)*(ncol(rxba_pval)-1)),countNotExpectedCellTypes(M2,rownames(dist),families),countNotExpectedCellTypes(M3,rownames(dist),families),sep='\t'))

  parents = families[rownames(dist),2]
  other_cell_types = setdiff(rownames(M2),c(rownames(dist),parents))
  p1 = pvals[rownames(dist),]
  p2 = pvals[other_cell_types,]
  message(paste(mix_set,mean(diag(rxba)),mean(diag(rxba_pval)),mean(M2[other_cell_types,]>0.001),mean(M2[other_cell_types,]>0.001 & p2<pval.thres),mean(M2[rownames(dist),]>0.001),mean(M2[rownames(dist),]>0.001 & p1<pval.thres),sep='\t'))
  ...
  if (do.ciber==TRUE || do.known==TRUE) {
    ...
    } else if (do.known==TRUE) {
      tl.col = rep('black',dim(rk$mat)[2])
    } else {
      tl.col = rep('blue',dim(rc$r2)[2])
    }
...
mcp.analyze = function(mix_set) {
  ...
  #mono =rownames(families)[families$Type=='Myeloid']
...
compareCorrelations = function(br,usr,uct,fn,w,h,drop.only1=TRUE,transpose=FALSE,font.size=7,ct_order=NULL) {
  ...
  if(drop.only1==TRUE) {
    mat = as.matrix(mat[,n>1])
    colnames(mat) = uct[n>1]
  }
  mat = as.matrix(mat[!apply(mat, 1, function(x){sum(!is.na(x))==0}),])
...
analyze.facs = function(fcs,scores,sets,known=FALSE,spill=NULL,alpha=0.5,beta_dist=NULL,fn=NULL) {
  ...
  if (is.null(spill)) {
    ...
    if (!is.null(beta_dist)) {
      beta_dist = beta_dist[rownames(s),]
      pvals = s
      for (i in 1:nrow(beta_dist))
        pvals[i,] = 1-unlist(lapply(s[i,],FUN=function(x) mean(x>beta_dist[i,])))
      s[pvals>0.05] = 0
...
fit.scores.plots = function(fv,families,working.dir,ctrl1,ctrl2,ctrl2_type=NULL,platform) {
  ...
  for (j in 1:3) {
    ...
    for (k in 1:N) {
      ...
      #z <- nls(y ~ a * x^b, data = d, start = list(a=1, b=1),control = list(maxiter = 500))
      #d$z = predict(z,d$x)
...
countNotExpectedCellTypes = function(scores,cell_types,families) {
  parents = families[cell_types,2]
  A = setdiff(rownames(scores),c(cell_types,parents))
  mean(scores[A,]>0.01)
}
